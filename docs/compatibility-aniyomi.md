...
| Cookies across requests                             | `JvmCookieJar` + SQLite, survives restart                                                      | **Supported** — per-plugin, per-host, in memory, gone at unload (`ABI.md` §2)            | Built. The relay stays stateless: the jar is host-side, `cookie` stays out of the forwardable allowlist, and `loadForRequest` is refused so a plugin cannot read it                                       | platform bridge              |
| Redirects                                           | okhttp default                                                                                 | **Supported** — followed one hop at a time with the target re-checked                    | —                                                                                                                                                                                                         | platform bridge              |
| okhttp `Interceptor`                                | seven real ones                                                                                | **Partly answered** — the named declarative ones translate, the rest stay refused        | Done for rate limiting (`FOREIGN.md` §4.1.8). UA is the host's value and not the extension's; gzip and double-encoded JSON are not interceptor questions here                                             | platform bridge + translator |
| Rate limiting                                       | `RateLimitInterceptor`, `SpecificHostRateLimitInterceptor`                                     | **Supported** — declared by the plugin, paced by the host (`ABI.md` §2.1)                | Was a no-op that returned its receiver: a limit accepted and dropped. Now a sliding window the host holds, with the period resolved exactly or the call refused                                           | shared runtime               |
